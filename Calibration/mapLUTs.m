...
% 2. Establish "Full Scale" boundaries for proper fractional conversion
targetSize = 256;
origSize = length(ChA1LUT_orig);

% Calculate the max theoretical inputs (Indices count minus 1)
orig_full_scale = origSize - 1;   % 1040
target_full_scale = targetSize - 1; % 255

% Setup unified normalized spatial axes [0, 1] for interpolation
x_orig = linspace(0, 1, origSize);
x_new = linspace(0, 1, targetSize);

% 3. Interpolate the original geometry smoothly
ChA1LUT_interpolated = interp1(x_orig, ChA1LUT_orig, x_new, 'pchip');

% 4. Turn values into fractions (e.g. 1030 / 1040 = ~0.99) and map to 8-Bit full scale (255)
fractional_curve = ChA1LUT_interpolated / orig_full_scale;
ChA1LUT_8bit = round(fractional_curve * target_full_scale);

% Clamp strictly to 8-bit limits [0, 255]
ChA1LUT_8bit = max(0, min(255, ChA1LUT_8bit));

% 5. Print out properly formatted C++ code
fprintf('==================== C++ ARRAY GENERATION ====================\n\n');
fprintf('const uint8_t PROGMEM ChA2LUT[%d] = {\n  ', targetSize);

for i = 1:targetSize
    fprintf('%d', ChA1LUT_8bit(i));
    
    if i < targetSize
        fprintf(', ');
    end
    
    % Break line every 16 values perfectly
    if mod(i, 16) == 0 && i ~= targetSize
        fprintf('\n  ');
    end
end
fprintf('\n};\n\n');
...
% 6. Plotting Diagnostics (Reflecting Percentage of Scale)
figure('Name', 'Gamma LUT Fractional Curve Alignment', 'Position', [100, 100, 1000, 450]);

% --- Subplot 1: Normalized Output Comparison ---
% Both plots compare Output Percentage against Input Percentage
subplot(1, 2, 1);
plot(x_orig, ChA1LUT_orig / orig_full_scale, 'b-', 'LineWidth', 2);
hold on;
plot(x_new, ChA1LUT_8bit / target_full_scale, 'r--', 'LineWidth', 2);
hold off;

xlabel('Normalized Input Position (0.0 to 1.0)');
ylabel('Output Fraction of Full Scale (0.0 to 1.0)');
title('Fractional Scale Comparison');
legend('Original LUT (max 99%)', 'New 8-bit LUT (max 99%)', 'Location', 'best');
grid on;

% --- Subplot 2: Accuracy/Error Analysis ---
subplot(1, 2, 2);

% Re-interpolate the new 8-bit fractions back to original resolution 
% to compute quantization error as a percentage.
ChA_8bit_upscaled = interp1(x_new, ChA1LUT_8bit / target_full_scale, x_orig, 'pchip');

% Error in fraction translated to % of the scale
percent_error = ((ChA1LUT_orig / orig_full_scale) - ChA_8bit_upscaled) * 100;

plot(x_orig, percent_error, 'k-', 'LineWidth', 1.5);
xlabel('Normalized Input Position');
ylabel('Error (% of Full Scale)');
title('Quantization Deviation');
grid on;